...
def read_and_resize(filepath):
    ...
    den = cv2.fastNlMeansDenoising(im_array)
    noise = den - im_array
    pil_im = Image.fromarray(noise)
    new_array = np.array(pil_im.resize((256, 256)))
 #   h , w = noise.shape[:2]
#    return res
    return new_array/255
    
print(read_and_resize(filepath))
...
#
# print(X_train.shape)
history = model.fit(X_train, y, validation_split=0.1, epochs=50, shuffle=True, verbose=2,callbacks=callbacks_list)
print('history')
# print(history)
#
